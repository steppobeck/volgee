...
namespace oglrwperf {
  ...
  timevalue::timevalue (const int& secs, const unsigned int& nsecs)
  {
    _stamp.tv_sec = static_cast<timevalue_rep::sec_type> (secs);
    _stamp.tv_nsec = static_cast<timevalue_rep::nsec_type> (nsecs);
  ...
  timevalue::operator timevalue::timevalue_rep () const
  {
    return _stamp;
  }

  const timevalue& timevalue::operator= (const timevalue& a)
  {
    _stamp = a._stamp;
  ...
  const timevalue& timevalue::operator= (const double& a)
  {
    typedef timevalue_rep::sec_type sec_type;
    typedef timevalue_rep::nsec_type nsec_type;
  ...
  timevalue& timevalue::operator+= (const timevalue& a)
  {
    _stamp.tv_sec += a._stamp.tv_sec;
    _stamp.tv_nsec += a._stamp.tv_nsec;
  ...
  timevalue& timevalue::operator*= (const unsigned int& a)
  {
    if (0 != a) {
      _stamp.tv_sec *= a;
      _stamp.tv_nsec *= a;
  ...
  timevalue& timevalue::operator/= (const unsigned int& a)
  {
    _stamp.tv_nsec += _stamp.tv_sec * one_second_in_nsec;
    _stamp.tv_sec = 0;
    ...
    if (0 !=  a) {
      ...
      std::cerr << "<timevalue::operator/=>: "
                << "applying division by zero "
                << "provides a time value of {"
                << _stamp.tv_sec << ","
                << _stamp.tv_nsec << "} ..."
                << std::endl;
    }
  ...
  std::string timevalue::formatted () const
  {
    std::size_t current_secs (_stamp.tv_sec);
    ...
    result << std::setfill ('0')
           ...
           << std::setw (9) << _stamp.tv_nsec;

    return result.str ().c_str ();
  }

  timevalue::size_type timevalue::sec () const
  {
    return (nsec () / one_second_in_nsec);
  }

  timevalue::size_type timevalue::msec () const
  {
    return (nsec () / one_second_in_usec);
  }

  timevalue::size_type timevalue::usec () const
  {
    return (nsec () / one_second_in_msec);
  }
  
  timevalue::size_type timevalue::nsec () const
  {
  ...
  bool operator< (const timevalue& a, const timevalue& b)
  {
    if ((a._stamp.tv_sec < b._stamp.tv_sec) ||
        ((a._stamp.tv_sec == b._stamp.tv_sec) &&
         (a._stamp.tv_nsec < b._stamp.tv_nsec)))
      return true;
  ...
  void nanosleep (const timevalue& timeout)
  {
...
# error "unable to determine system time api"
  ...
  void sleep (const timevalue& timeout)
  {
    nanosleep (timeout);
  }
  ...
  {
    if (one_second_in_nsec <= _stamp.tv_nsec) {
      do {
        _stamp.tv_sec++;
        _stamp.tv_nsec -= one_second_in_nsec;
      } while (one_second_in_nsec <= _stamp.tv_nsec);
    } else if (one_second_in_nsec_neg >= _stamp.tv_nsec) {
      do {
        _stamp.tv_sec--;
        _stamp.tv_nsec += one_second_in_nsec;
      } while (one_second_in_nsec_neg >= _stamp.tv_nsec);
    }

    if ((1 <= _stamp.tv_sec) && (0 > _stamp.tv_nsec)) {
      _stamp.tv_sec--;
      _stamp.tv_nsec += one_second_in_nsec;
    } else if ((0 > _stamp.tv_sec) && (0 < _stamp.tv_nsec)) {
      _stamp.tv_sec++;
      _stamp.tv_nsec -= one_second_in_nsec;
    }
  }

  timevalue operator+ (const timevalue& a, const timevalue& b)
  {
  ...
  timevalue operator* (const timevalue& a, const unsigned int& b)
  {
    return timevalue (a) *= b;
  }

  timevalue operator* (const unsigned int& a, const timevalue& b)
  {
  ...
  std::ostream& operator<< (std::ostream& os, const timevalue& a)
  {
